    ...
    static void enable_config_options(size_t index = -1)
    {
      pfasst::config::Options::get_instance()
        .register_init_function("Boris-SDC",
                                std::function<void(po::options_description&)>(examples::boris::init_config_options<scalar>),
                                index);
    }
...
//           this->particles[m]->accel() = this->particles[m]->alpha() * (e + acceleration_type(b));
        ...
        virtual void sweep() override
        {
          ...
          for (size_t m = 0; m < nnodes - 1; m++) {
            ...
            for (size_t l = 0; l <= m; l++) {
              this->particles[m+1]->positions() += this->forces[l] * dt * dt * this->sx_mat(m+1, l);
              this->particles[m+1]->positions() -= this->previous_forces[l] * dt * dt * this->sx_mat(m+1, l);
            }
            ...
            c_k_term -= 0.5 * this->previous_forces[m+1] * ds;
            //                 - delta_nodes_{m} / 2 * f_{m}^{k}
            c_k_term -= 0.5 * this->previous_forces[m] * ds;
            //                 + s_integral[m]
        ...
        //! @{
        virtual void post(pfasst::ICommunicator* comm, int tag) override
        {
          UNUSED(comm); UNUSED(tag);
          // TODO: implement BorisSweeper::post
        };

        virtual void send(pfasst::ICommunicator* comm, int tag, bool blocking) override
        {
          UNUSED(comm); UNUSED(tag); UNUSED(blocking);
          // TODO: implement BorisSweeper::send
        }

        virtual void recv(pfasst::ICommunicator* comm, int tag, bool blocking) override
        {
          UNUSED(comm); UNUSED(tag); UNUSED(blocking);
          // TODO: implement BorisSweeper::recv
        }

        virtual void broadcast(pfasst::ICommunicator* comm) override
        {
          UNUSED(comm);
                                 ...
                                 const velocity_type& c_k_term)
        {
          ...
          acceleration_type e_forces_mean = (this->impl_solver->e_field_evaluate(this->particles[m], tm) + this->forces[m+1]) / scalar(2.0);
